...
export default function ProfileGroupPreview({
  groupId,
  groupName,
  lastMessage,
  lastMessageDate,
  ...
  const handleGroupClick = async () => {
    try {
      ...
      if (window.innerWidth < 768) {
        ...
        router.push(`/groups?providedGroupId=${groupId}`)
      }
    } finally {
      setLoading(false)
    }
  ...
  return (
    ...
    >
      ...
      <div className="ml-3 flex-1 min-w-0">
        ...
        <div className="text-left text-sm md:text-base lg:text-lg xl:text-xl text-slate-600 line-clamp-2 leading-tight min-h-[30px]">
          {lastMessage}
        </div>
      </div>
    </div>
