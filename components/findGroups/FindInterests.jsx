...
export default function FindInterests({ onDone, userData }) {
  ...
  return (
    <div className='flex flex-col items-center w-full p-4'>
      <h1 className='font-bold text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl text-slate-900 mb-6'>Choose up to 3 topics</h1>
      <div className='flex flex-wrap gap-2 justify-center mb-6'>
        {selectedInterests.map((interest) => (
          <span
            key={interest}
            onClick={() => handleRemove(interest)}
            className='bg-blue-600 text-white px-3 py-1 rounded-full cursor-pointer text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl'
          >
            {interest} &times;
          </span>
        ))}
      ...
      <div className="flex flex-wrap gap-2 border border-slate-300 rounded p-2 mb-6">
        {INTERESTS.map((interest) => (
          <button
            ...
            className={`rounded-full px-3 py-2 text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl font-medium 
      ...
      <button
        ...
        className='text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl bg-blue-600 text-white py-2 px-4 lg:py-4 lg:px-8 rounded-full font-semibold w-full max-w-[300px] disabled:opacity-50 cursor-pointer'
      >
        Continue
      </button>
    </div>
