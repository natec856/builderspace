...
export default function FindExperience({ onDone, onBack, userData }) {
  ...
  return (
    <div className='flex flex-col items-center w-full'>
      <h1 className='font-bold text-xl sm:text-2xl md:text-3xl lg:text-4xl xl:text-5xl mb-6'>
        ...
        {Object.entries(EXPERIENCE).map(([level, label]) => (
          <li key={level}>
            <button
              ...
              className={`w-full text-center py-2 px-4 lg:py-4 lg:px-8 rounded border font-medium cursor-pointer text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl 
                ${selected === level
      ...
      <button
        ...
        disabled={!selected}
        className='text-base sm:text-lg md:text-xl lg:text-2xl xl:text-3xl bg-blue-600 text-white py-2 px-4 lg:py-4 lg:px-8 rounded-full font-semibold w-full max-w-[300px] disabled:opacity-50 cursor-pointer mt-6'
      >
        Continue
      </button>
      <button
