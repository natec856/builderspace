...
export default function InvitesPreview({ invite_id, username, name, avatar_url, currentUserUsername, currentUserId }) {
  return (
    <div className="flex items-center justify-between w-full gap-3 border-b border-slate-200 py-2 min-h-[80px]">
      {/* User Info container */}
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="flex flex-col min-w-0">
          <Link
            href={`/${username}`}
            className="font-bold text-base sm:text-lg md:text-xl text-slate-900 truncate hover:underline"
          >
            {name}
          </Link>
          <div className="text-sm sm:text-base md:text-lg  text-slate-600 truncate">
            @{username}
          </div>
        </div>
      </div>
